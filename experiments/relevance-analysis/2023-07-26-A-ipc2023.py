...
BENCHMARKS_DIR = os.environ["IPC2023_BENCHMARKS_SAT"]
...
CONFIGS = [Configuration('baseline', ['--grounder', 'gringo', '--lpopt-preprocessor'], False),
           Configuration('relevance-analysis', ['--grounder', 'gringo', '--lpopt-preprocessor'], True)]
...
for config in CONFIGS:
    for task in suites.build_suite(BENCHMARKS_DIR, SUITE):
        ...
        if config.relevance_analysis:
           run.add_command('run-relevance-analysis',
                           [RUN_SCRIPT_DIR+'/relevance-analysis.py', '-i', task.problem_file,
                            '-m', model_name,
                            '--lpopt-preprocessor'],
                           time_limit=TIME_LIMIT,
                           memory_limit=MEMORY_LIMIT)
...
def set_relevant_atoms(run):
    if "relevant_atoms" not in run:
        run["relevant_atoms"] = run["non_temporary_atoms"]
    return run
